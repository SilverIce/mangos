...
FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    const Terrain * _map = owner.GetTerrain();
    ...
    for(uint8 i = 0; i < 18; ++i)
    {
        ...
        if( owner.IsWithinLOS(temp_x,temp_y,z))
        {
            ...
            {
                float new_z_left = _map->GetHeight(owner.GetPhaseMask(),temp_x + 1.0f*cos(angle+M_PI_F/2),temp_y + 1.0f*sin(angle+M_PI_F/2),z,true);
                float new_z_right = _map->GetHeight(owner.GetPhaseMask(),temp_x + 1.0f*cos(angle-M_PI_F/2),temp_y + 1.0f*sin(angle-M_PI_F/2),z,true);
