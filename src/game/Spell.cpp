...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_timer != 0) &&
        ...
        (m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK || !m_caster->movement->IsFallingFar()))
    {
        ...
        case SPELL_STATE_CASTING:
        {
            if(m_timer > 0)
            {
                if( m_caster->GetTypeId() == TYPEID_PLAYER )
                {
                    ...
                    if (m_caster->movement->IsFalling())
                        cancel();
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (((Player*)m_caster)->isMoving() )
        {
            ...
            if ((!m_caster->movement->IsFallingFar() || m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK) &&
